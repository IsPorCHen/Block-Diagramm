"""Statement handlers for C# AST."""

from typing import List, Dict, Any, Tuple, Union
from app.parsers.core.graph_builder import GraphBuilder


class CSharpHandlers:
    """Handles C# statements and builds flowchart nodes."""
    ...
    def _handle_statement(self, stmt: Dict[str, Any], 
        ...
        if stmt_type in ('class', 'struct', 'interface', 'enum'):
            return prev_ids  # Skip type declarations inside body
        elif stmt_type == 'if':
            return self._handle_if(stmt, prev_ids)
        elif stmt_type in ('for', 'foreach'):
            ...
            return self._handle_break_continue(stmt, prev_ids)
        elif stmt_type in ('using_directive', 'using_statement'):
            return self._handle_using(stmt, prev_ids)
    ...
    def _handle_for(self, stmt: Dict[str, Any], 
                    prev_ids: List[Union[int, Tuple]]) -> List[Union[int, Tuple]]:
        """Handle for/foreach loop."""
        loop_type = stmt.get('type', 'for')
        header = stmt.get('header', '...')
        
        label = f'{loop_type} ({header})'
        loop_id = self._add_node('loop', label, prev_ids)
    ...
    def _handle_do_while(self, stmt: Dict[str, Any], 
        ...
        for node in self.graph._nodes:
            if node.type in ('process', 'input', 'output', 'loop'):
    ...
    def _handle_break_continue(self, stmt: Dict[str, Any], 
                               prev_ids: List[Union[int, Tuple]]) -> List[Union[int, Tuple]]:
        """Handle break or continue."""
        stmt_type = stmt.get('type', 'break')
        self._add_node('process', stmt_type, prev_ids)
        return [None]
    
    def _handle_using(self, stmt: Dict[str, Any], 
                      prev_ids: List[Union[int, Tuple]]) -> List[Union[int, Tuple]]:
        """Handle using statement."""
        value = stmt.get('value', '')
        
        if stmt.get('type') == 'using_statement':
            # using with block - treat as try/finally
            using_id = self._add_node('process', f'using ({value})', prev_ids)
            body = stmt.get('body', [])
            if body:
                body_ids = self.process_body(body, [using_id])
                return body_ids
            return [using_id]
        else:
            # using directive - skip
            return prev_ids
    ...
    def _handle_expression(self, stmt: Dict[str, Any], 
        ...
        if value:
            ...
            if 'Console.Write' in value or 'Console.WriteLine' in value:
